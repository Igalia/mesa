...
void emit_instruction(asm_context ctx, std::vector<uint32_t>& out, Instruction* instr)
{
   ...
   case Format::EXP: {
      ...
      out.push_back(encoding);
      encoding = (uint64_t) exp->getOperand(0).physReg().reg;
      encoding |= (uint64_t) exp->getOperand(1).physReg().reg << 8;
      encoding |= (uint64_t) exp->getOperand(2).physReg().reg << 16;
      encoding |= (uint64_t) exp->getOperand(3).physReg().reg << 24;
      out.push_back(encoding);
      break;
   }
   default:
